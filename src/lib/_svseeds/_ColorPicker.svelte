...
<script lang="ts">
  // prettier-ignore
  let { value = $bindable(DEFAULT_COLOR), alpha = 1, attach, element = $bindable(), styling, variant = VARIANT.NEUTRAL, class: c, ...rest }: ColorPickerProps = $props();
  ...
  // *** Bind Handlers *** //
  const rgb = $derived(castHexToRgb(value));
...
<label class={cls(PARTS.WHOLE, variant)}>
  <div
    ...
    style="display: inline-block; background-image: linear-gradient(45deg,#ccc 25%,transparent 25%),linear-gradient(-45deg,#ccc 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#ccc 75%),linear-gradient(-45deg,transparent 75%,#ccc 75%); background-size: 20px 20px; background-position: 0 0,0 10px,10px -10px,-10px 0px;"
  >
    <div class={cls(PARTS.MAIN, variant)} style={`background-color: rgba(${rgb[0]},${rgb[1]},${rgb[2]},${alp})`}>
      <input bind:value bind:this={element} class={c} {...rest} type="color" style="visibility: hidden;" {@attach attach} />
    </div>
  </div>
</label>
