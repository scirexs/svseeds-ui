...
- bun-run-fmt-rewrites-whole-src-lib.md — `bun run fmt` runs `prettier --write ./src/lib` over the whole tree; on a scoped task it picks up unrelated pre-existing formatting churn — use a scoped `prettier --check` instead.
