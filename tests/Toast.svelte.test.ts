...
const dialogs = (c: HTMLElement) => [...c.querySelectorAll('[role="dialog"]')] as HTMLElement[];
...
describe("Toaster management", () => {
  ...
  test("[T-6] dismiss() then removal", async () => {
    const { toaster, container } = setup(); const el = region(container);
    const id = toaster.add("Test message");
    await expect.element(el).toHaveAttribute("aria-label", "1 notifications");
    toaster.dismiss(id);
    await expect.element(el).toHaveAttribute("aria-label", "0 notifications");
  ...
  test("[T-7] multiple add/dismiss", async () => {
    const { toaster, container } = setup(); const el = region(container);
    toaster.add("Message 1", { type: "info" }); const id = toaster.add("Message 2", { type: "warning" }); toaster.add("Message 3", { type: "error" });
  ...
  test("[T-11] clear() dismisses all", async () => {
    const { toaster, container } = setup(); const el = region(container);
    toaster.add("Message 1"); toaster.add("Message 2"); toaster.add("Message 3");
    await expect.element(el).toHaveAttribute("aria-label", "3 notifications");
    toaster.clear();
    await expect.element(el).toHaveAttribute("aria-label", "0 notifications");
  });
...
describe("Toast display and interaction", () => {
  test("[T-17] children receives item and leave state uses effective variant", async () => {
    ...
    expect(el.textContent).toContain("Display message"); expect(el.textContent).toContain("success"); expect(el.textContent).toContain(id);
    toaster.dismiss(id); await tick();
    await expect.element(dialogs(container)[0]).toHaveClass(PARTS.MAIN, VARIANT.INACTIVE);
  });
  test("[T-18] children snippet receives updated effective variant", async () => {
    const toaster = createToaster(); const { container } = render(ToastSnippetProbe, { toaster, variant: "primary" });
    const id = toaster.add("Compiled snippet", { type: "success" }); await tick();
    await expect.element(dialogs(container)[0]).toHaveClass(PARTS.MAIN, "primary");
  ...
  test("[T-19] type feeds dialog aria-label", async () => {
    const { toaster, container } = setup(); toaster.add("Typed", { type: "error" }); toaster.add("Untyped"); await tick();
    await expect.element(dialogs(container)[0]).toHaveAttribute("aria-label", "error message");
    await expect.element(dialogs(container)[1]).toHaveAttribute("aria-label", "message");
  });
});

describe("Keyboard navigation", () => {
  test("[T-23] F6 key focuses toast container", async () => {
    const { toaster, container } = setup(); const el = region(container); toaster.add("Focus message");
    await expect.element(el).toHaveAttribute("aria-label", "1 notifications");
    document.body.dispatchEvent(new KeyboardEvent("keydown", { key: "F6", bubbles: true, composed: false })); await tick();
    await expect.element(el).toHaveFocus();
  });
  test("[T-24] F6 key with composed event is ignored", async () => {
    const { toaster, container } = setup(); const el = region(container); toaster.add("Composed message");
    await expect.element(el).toHaveAttribute("aria-label", "1 notifications");
    document.body.dispatchEvent(new KeyboardEvent("keydown", { key: "F6", bubbles: true, composed: true })); await tick();
    expect(document.activeElement).not.toBe(el);
  });
  test("[T-25] other keys are ignored", async () => {
    const { toaster, container } = setup(); const el = region(container); toaster.add("Other key message");
    await expect.element(el).toHaveAttribute("aria-label", "1 notifications");
    document.body.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true, composed: false }));
    document.body.dispatchEvent(new KeyboardEvent("keydown", { key: "Enter", bubbles: true, composed: false })); await tick();
    expect(document.activeElement).not.toBe(el);
  });
});
...
describe("Styling and lifecycle", () => {
  ...
  test("[T-26] applies CSS classes correctly", async () => {
    const { toaster, container } = setup(undefined, { variant: VARIANT.NEUTRAL }); toaster.add("Styled message"); await tick();
    await expect.element(region(container)).toHaveClass(preset, PARTS.WHOLE, VARIANT.NEUTRAL);
    await expect.element(container.querySelector(`[class*="${PARTS.MIDDLE}"]`) as HTMLElement).toHaveClass(preset, PARTS.MIDDLE, VARIANT.NEUTRAL);
    await expect.element(dialogs(container)[0]).toHaveClass(preset, PARTS.MAIN, VARIANT.NEUTRAL);
  });
  test("[T-27] INACTIVE while leaving", async () => {
    const { toaster, container } = setup(undefined, { variant: "custom" }); const id = toaster.add("Hide message"); await tick();
    await expect.element(dialogs(container)[0]).toHaveClass(PARTS.MAIN, "custom"); toaster.dismiss(id);
    await expect.element(dialogs(container)[0]).toHaveClass(PARTS.MAIN, VARIANT.INACTIVE);
  });
  test("[T-28] custom styling", async () => {
    const { toaster, container } = setup(undefined, { styling: "custom-toast-style" }); toaster.add("Custom styled message"); await tick();
    await expect.element(region(container)).toHaveClass("custom-toast-style", PARTS.WHOLE);
  });
  test("[T-29] real Popover lifecycle", async () => {
    const { toaster, container } = setup(); const el = region(container); const id = toaster.add("Popover message");
    await expect.element(el).toHaveAttribute("aria-label", "1 notifications"); expect(el.matches(":popover-open")).toBe(true);
    toaster.dismiss(id); await expect.element(el).toHaveAttribute("aria-label", "0 notifications"); expect(el.matches(":popover-open")).toBe(false);
  });
  test("[T-30] flip duration from motion prop", async () => {
    const motion = 600;
    const { toaster, container } = setup({ duration: Infinity }, { motion });
    const el = region(container);
    const first = toaster.add("First message"); toaster.add("Second message"); await tick();
    await expect.element(el).toHaveAttribute("aria-label", "2 notifications");
    expect(dialogs(container)).toHaveLength(2);
    // dismiss flips the leaving dialog to INACTIVE immediately and defers its removal by `motion` (600ms)
