...
describe("Modal open and close behavior", () => {
  ...
  test("uses target-precise backdrop and content clicks", async () => {
    ...
    const backdrop = dialog(closeRender.container);
    await tick();
    backdrop.click();
    await tick();
    ...
    const fixedDialog = dialog(fixedRender.container);
    await tick();
    fixedDialog.click();
    await tick();
    expect(fixed.open).toBe(true);
    const content = $state({ children: childrenSnippet, open: true, closable: true });
    const contentRender = render(Modal, content);
    await tick();
    await userEvent.click(byText(contentRender.container, childrenText));
    await tick();
  ...
  test("handles Escape and native toggle synchronization", async () => {
    ...
    const blockedDialog = dialog(blocked.container);
    await tick();
    const escape = new KeyboardEvent("keydown", { key: "Escape", bubbles: true, cancelable: true });
    blockedDialog.dispatchEvent(escape);
    expect(escape.defaultPrevented).toBe(true);
    expect(blockedDialog.open).toBe(true);
    ...
    dialog(allowed.container).dispatchEvent(allowedEscape);
    expect(allowedEscape.defaultPrevented).toBe(false);
    const props = $state({ children: childrenSnippet, open: true });
    const synced = render(Modal, props);
    const syncedDialog = dialog(synced.container);
    await tick();
    syncedDialog.open = false;
    syncedDialog.dispatchEvent(new Event("toggle"));
    await tick();
...
describe("Modal styling and duration", () => {
  test("applies variants, styling, structure, and reactive variant changes", async () => {
    ...
    await tick();
    await expect.element(el).toHaveClass(preset, PARTS.WHOLE, VARIANT.ACTIVE);
    props.open = false;
    await tick();
    ...
    await tick();
    await expect.element(el).toHaveClass(preset, PARTS.WHOLE, VARIANT.ACTIVE);
    const styled = render(Modal, { children: childrenSnippet, open: true, styling: "custom-modal" });
    await tick();
    await expect.element(dialog(styled.container)).toHaveClass("custom-modal", PARTS.WHOLE, VARIANT.NEUTRAL);
    const objectStyled = render(Modal, {
      children: childrenSnippet,
      open: true,
      styling: { whole: { base: "modal-base", neutral: "modal-neutral" } },
    });
    await tick();
    await expect.element(dialog(objectStyled.container)).toHaveClass("modal-base", "modal-neutral");
    const custom = render(Modal, { children: childrenSnippet, open: true, variant: "custom-variant" });
    await tick();
  ...
  test("emits normal, reduced-motion, renamed, and invalid duration values", async () => {
    const normal = render(Modal, { children: childrenSnippet, open: true });
    await tick();
    expect(dialog(normal.container).style.getPropertyValue("--svs-duration")).toBe("200ms");
    const custom = render(Modal, { children: childrenSnippet, open: true, duration: 500 });
    await tick();
    expect(dialog(custom.container).style.getPropertyValue("--svs-duration")).toBe("500ms");
    const renamed = render(Modal, { children: childrenSnippet, open: true, cssvar: { duration: "--x" } });
    await tick();
    ...
    const invalidRender = render(Modal, invalid);
    const invalidDialog = dialog(invalidRender.container);
    await tick();
    expect(invalidDialog.style.getPropertyValue("--svs-duration")).toBe("200ms");
    invalid.duration = NaN;
    await tick();
    expect(invalidDialog.style.getPropertyValue("--svs-duration")).toBe("200ms");
    invalid.duration = 1.5;
    await tick();
    expect(invalidDialog.style.getPropertyValue("--svs-duration")).toBe("200ms");
    ...
    const reduced = render(Modal, { children: childrenSnippet, open: true });
    await tick();
...
describe("Modal event passthrough and edge cases", () => {
  test("runs passthrough handlers before its own close, key, and toggle behavior", async () => {
    const onclick = vi.fn();
    const clickProps = $state({ children: childrenSnippet, open: true, closable: true, onclick });
    const clicked = render(Modal, clickProps);
    const clickedDialog = dialog(clicked.container);
    await tick();
    clickedDialog.click();
    await tick();
    expect(onclick).toHaveBeenCalledTimes(1);
    expect(clickProps.open).toBe(false);
    const onkeydown = vi.fn();
    const keyed = render(Modal, { children: childrenSnippet, open: true, closable: false, onkeydown });
    const key = new KeyboardEvent("keydown", { key: "Escape", bubbles: true, cancelable: true });
    dialog(keyed.container).dispatchEvent(key);
    expect(onkeydown).toHaveBeenCalledTimes(1);
    expect(key.defaultPrevented).toBe(true);
    const ontoggle = vi.fn();
    const toggleProps = $state({ children: childrenSnippet, open: true, ontoggle });
    const toggled = render(Modal, toggleProps);
    const toggledDialog = dialog(toggled.container);
    await tick();
    toggledDialog.open = false;
    toggledDialog.dispatchEvent(new Event("toggle"));
    await tick();
    expect(ontoggle).toHaveBeenCalledTimes(1);
    expect(toggleProps.open).toBe(false);
    const blockedClick = vi.fn();
    const blockedProps = $state({ children: childrenSnippet, open: true, closable: false, onclick: blockedClick });
    const blocked = render(Modal, blockedProps);
    await tick();
    dialog(blocked.container).click();
    await tick();
    expect(blockedClick).toHaveBeenCalledTimes(1);
    expect(blockedProps.open).toBe(true);
  });

  test("reopens synchronously and supports complex children", async () => {
    const props = $state({ children: childrenSnippet, open: true });
    const rendered = render(Modal, props);
    const el = dialog(rendered.container);
    await tick();
    props.open = false;
    await tick();
    props.open = true;
    ...
    const contents = render(Modal, { children: complex, open: true });
    await tick();
