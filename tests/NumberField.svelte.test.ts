...
const snippet = (id: string) =>
  createRawSnippet((value: () => number | undefined, variant: () => string, element: () => NumberFieldElement) => ({
...
describe("NumberField chrome", () => {
  test("renders label, extra, group labelling, snippets, and bottom description", async () => {
    ...
    await expect
      .element(container.querySelector(`[data-testid="${rightid}"]`)!.parentElement!)
      .toHaveClass("svs-number-field", PARTS.RIGHT);
    ...
    await expect
      .element(container.querySelector(`[data-testid="${leftid}"]`) as HTMLElement)
      .toHaveTextContent(`4:${VARIANT.NEUTRAL}:none`);
    await expect
      .element(container.querySelector(`[data-testid="${rightid}"]`) as HTMLElement)
      .toHaveTextContent(`4:${VARIANT.NEUTRAL}:none`);
  });
...
describe("NumberField default child and binding", () => {
  test("forwards input configuration, snippets, stack, and children replacement", async () => {
    ...
    const snippets = render(NumberField, { numberInput: { spin: "stack", left: decrement, right: increment } });
  ...
  test("syncs values and programmatic updates", async () => {
    ...
    programmatic.value = 8;
    await programmaticRender.rerender(programmatic);
  ...
  test("preserves min/max/step validation through NumberInput", async () => {
    ...
    await expect.element(el).toHaveValue("6");
  ...
  test("re-runs custom validations and drives variant when the spinner bumps the value", async () => {
    const props = $state({
      ...
      numberInput: { spin: "split" as NumberInputSpin, min: 0, max: 10, step: 2 },
    });
    const { container } = render(NumberField, props);
    const el = input(container);
...
describe("NumberField variant state machine and a11y", () => {
  ...
  test("shows, wires, and clears validation errors; invalid events mark inactive", async () => {
    ...
    const { container } = render(NumberField, props);
    const el = input(container);
    await userEvent.type(el, "3");
    await change(el);
    const alert = container.querySelector('[role="alert"]') as HTMLElement;
    expect(props.variant).toBe(VARIANT.INACTIVE);
    expect(alert.textContent).toBe(errmsg);
    await expect.element(el).toHaveAttribute("aria-invalid", "true");
    await expect.element(el).toHaveAccessibleErrorMessage(errmsg);
    await userEvent.clear(el);
    await userEvent.type(el, "7");
    await change(el);
    expect(props.variant).toBe(VARIANT.ACTIVE);
    ...
    const invalidInput = input(invalidRender.container);
    await tick();
    await invalid(invalidInput);
  ...
  test("wires describedby and errormessage IDs", async () => {
    ...
    const { container } = render(NumberField, props);
    const el = input(container);
    const desc = container.querySelector(`.${PARTS.BOTTOM}`) as HTMLElement;
