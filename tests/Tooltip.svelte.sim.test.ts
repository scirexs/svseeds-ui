...
describe("Touch long-press fallback retained in jsdom", () => {
  test("[TT-11] anchors to the press point on touch even when cursor is false", async () => {
    ...
    await advance(10);
    expect(getTip().style.left).toBe("70px");
    expect(getTip().style.top).toBe("70px");
  });
});
...
describe("Positioning and flipping retained in jsdom", () => {
  ...
  test("[TT-25] positions at top-center by default", async () => {
    const el = await showAt({ x: 200, y: 200, width: 100, height: 40 });
    expect(el.style.left).toBe("220px");
    expect(el.style.top).toBe("170px");
  });
  test("[TT-26] flips from top to bottom when there is no room above", async () => {
  ...
  test("[TT-28] offset is not shared between calls", async () => {
...
describe("Cursor tracking retained in jsdom", () => {
  ...
  test("[TT-29] anchors to the pointer position when cursor is enabled", async () => {
    const trigger = makeTrigger();
  ...
  test("[TT-30] follows pointermove that happens before the tooltip becomes visible", async () => {
    const trigger = makeTrigger();
    ...
    await tick();
    await advance(100);
    expect(getTip().style.left).toBe("120px");
    expect(getTip().style.top).toBe("120px");
  });
});
